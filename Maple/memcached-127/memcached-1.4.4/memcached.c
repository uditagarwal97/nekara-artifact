...
#include <ctype.h>
#include <stdarg.h>
#include <include_coyote/coyote_c_ffi.h>
...
volatile static int transaction_completed = 0; // Udit
...
__attribute__((constructor)) static void register_coyote_memcache(){
...
conn *conn_from_freelist() {
    ...
    FFI_pthread_mutex_lock(&conn_lock);
    if (freecurr > 0) {
        c = freeconns[--freecurr];
    } else {
        c = NULL;
    }
    FFI_pthread_mutex_unlock(&conn_lock);
...
static void complete_nread_ascii(conn *c) {
    ...
    FFI_pthread_mutex_lock(&c->thread->stats.mutex);
    c->thread->stats.slab_stats[it->slabs_clsid].set_cmds++;
    FFI_pthread_mutex_unlock(&c->thread->stats.mutex);
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    if(transaction_completed == 0){
        char str[50] = "set key 01 0 1\r\n";

        res = strlen(str);

        memcpy((char *)c->rbuf, str, res);
        c->rcurr = c->rbuf;
        c->rbytes = res;
        c->request_id = 1;

        FFI_pthread_mutex_lock(&c->thread->stats.mutex);
        c->thread->stats.bytes_read += (res + 8);
        FFI_pthread_mutex_unlock(&c->thread->stats.mutex);

    } else {

        if(transaction_completed == 1 || transaction_completed == 2){
            char str[50] = "incr key 10\r\n";
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        ...
        if(transaction_completed == 0){
            ...
            res = strlen(str);
            memcpy((char *)(c->rbuf + c->rbytes), str, res);
            ...
            if(transaction_completed == 1 || transaction_completed == 2){
                ...
                assert(transaction_completed == 3);
...
static enum transmit_result transmit(conn *c) {
	...
	// assert(0);
        ...
        // ret val for set()
        if(!transaction_completed && !strcmp((char *)(m->msg_iov[0].iov_base), "STORED\r\n")){
        ...
        if( (transaction_completed == 1) ){
            assert(0 == strcmp((char *)(m->msg_iov[0].iov_base), "11\r\n"));
        ...
        // ret val for incr
        if( (transaction_completed == 2) ){
            assert(0 == strcmp((char *)(m->msg_iov[0].iov_base), "21\r\n"));
	    printf("transaction 2 verified\n");
            transaction_completed = 3;
            goto complete;
        }

        // ret val for get
        if( (transaction_completed == 3) ){
            assert(((char *)m->msg_iov[2].iov_base)[6] == '2' && ((char *)m->msg_iov[2].iov_base)[7] == '1');
            // Final value should be 21
            transaction_completed = -1;
	    printf("transaction 3 verified\n");
            goto complete;
        }
...
int run_iteration (int argc, char **argv) {
    ...
    */
    ...
    event_handler(listen_conn); // For sending SET command
    /* enter the event loop */
    // event_base_loop(main_base, 0);
    // Udit's changes! 
    
    while(transaction_completed != 1){FFI_schedule_next();} //spinloop

    event_handler(listen_conn); // For sending 2 incr commands
    FFI_schedule_next();
    FFI_schedule_next();
    event_handler(listen_conn); 
    
    while(transaction_completed != 3){FFI_schedule_next();} //spinloop

    event_handler(listen_conn); // For sending a GET command and assert

    while(transaction_completed != -1){FFI_schedule_next();} // spin loop 

    // One Iteration completed!    

    stop_assoc_maintenance_thread();

    // Close all the worker threads!
...
int main (int argc, char **argv) {
	...
	assert(EXIT_SUCCESS == run_iteration(argc, argv));
